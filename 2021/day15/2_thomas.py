...
neighbors_to_check = []
heappush(neighbors_to_check, [0,0])
...
while(True):
    y,x = heappop(neighbors_to_check)
    if(x > 0):
        if(costs[y,x-1] > costs[y,x] + f[y,x-1] or costs[y,x-1] == 0):
            costs[y,x-1] = costs[y,x] + f[y,x-1]
            if([y,x-1] not in neighbors_to_check and [y,x-1] < [y,x]):
                heappush(neighbors_to_check, [y,x-1])
    ...
    if(y > 0):
        if(costs[y-1,x] > costs[y,x] + f[y-1,x] or costs[y-1,x] == 0):
            ...
            if([y-1,x] not in neighbors_to_check and [y-1,x] < [y,x]):
                heappush(neighbors_to_check, [y-1,x])
