...
def explode(arr,depth=0):
    ...
    if(type(arr[1]) == list and exploded_right == -1):
...
def write_exploded_number(arr,left,right):
    print(str(arr).replace("[","").replace("]","").replace(" ","").split(","))
...
while(True):
    ...
    write_exploded_number(arr,exploded_left,exploded_right)
