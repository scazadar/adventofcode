...
sizes = {tuple(map(int,_.split(":")[0].split("x"))):tuple(map(int,_.split(":")[1].split())) for _ in input[-1].split("\n")}
...
for grid in sizes:
    if(calcSize(grid,sizes[grid])):
        reducedPresentShapes.setdefault(grid,sizes[grid])
        
for k in reducedPresentShapes:
    print(k,reducedPresentShapes[k])
